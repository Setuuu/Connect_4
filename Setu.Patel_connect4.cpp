...
for (int c = 0; c < COLS; c++){
  ...
  for (int r = 0; r < ROWS; r++ ){
    if (board[c][r] == currTile){
      // Checks if winning diagonally is possible
      //If possible, check cooordinates diagonally
      if ((r +3 < ROWS) && (c + 3 < COLS)){
        //Going from top left to bottom right the r and c cooordinates both
        //increase by 1
        for (int i = 0; i <4; i++ ){
  ...
  for (int c = 0; c < COLS; c++){
    ...
    for (int r = 0; r < ROWS; r++ ){
      if (board[c][r] == currTile){
        //Checks if winning diagonally is possible
        //If possible checks cooordinates diagonally
        if ((r +3 < ROWS) && (c + 3 < COLS)){
          for (int i = 0; i <4; i++ ){
            //Going from top right to bottom left, the r cooordinates increase
            // by 1, and c decrease by 1
            if (board[c-i] [r+i] == currTile){
              counter++;
            } else {
              counter = 0;
